...
export default function AdminOrdersPage() {
  ...
  const getStatusBadge = (status: Order["status"]) => {
    switch (status) {
      ...
      case "PROCESSING":
        return <Badge variant="outline" className="bg-blue-50 text-blue-800 border-blue-200 dark:bg-blue-950/20 dark:text-blue-400 dark:border-blue-900 rounded-md">Processing</Badge>;
      case "SHIPPED":
        return <Badge variant="outline" className="bg-purple-50 text-purple-800 border-purple-200 dark:bg-purple-950/20 dark:text-purple-400 dark:border-purple-900 rounded-md">Shipped</Badge>;
      case "DELIVERED":
        return <Badge variant="outline" className="bg-emerald-50 text-emerald-800 border-emerald-200 dark:bg-emerald-950/20 dark:text-emerald-400 dark:border-emerald-800 rounded-md">Delivered</Badge>;
      case "CANCELLED":
        return <Badge variant="outline" className="bg-stone-50 text-stone-500 border-stone-200 dark:bg-stone-900 dark:text-stone-400 dark:border-stone-800 rounded-md">Cancelled</Badge>;
      case "FAILED":
      ...
      default:
        return <Badge variant="outline" className="bg-stone-100 text-stone-800 border-stone-300 dark:bg-stone-900 dark:text-stone-300 dark:border-stone-800 rounded-md">{status}</Badge>;
    }
  };
  ...
  const getPaymentStatusBadge = (status: Order["paymentStatus"]) => {
    switch (status) {
      case "PAID":
        return <Badge className="bg-emerald-600 dark:bg-emerald-500 text-white dark:text-stone-950 rounded-md hover:bg-emerald-700 dark:hover:bg-emerald-400">Paid</Badge>;
      case "PENDING":
        return <Badge className="bg-amber-500 dark:bg-amber-500 text-white dark:text-stone-950 rounded-md hover:bg-amber-600 dark:hover:bg-amber-400">Pending</Badge>;
      case "FAILED":
        return <Badge className="bg-rose-600 dark:bg-rose-500 text-white dark:text-stone-950 rounded-md hover:bg-rose-700 dark:hover:bg-rose-400">Failed</Badge>;
      default:
        return <Badge className="bg-stone-500 dark:bg-stone-500 text-white dark:text-stone-950 rounded-md">{status}</Badge>;
    }
  };
    ...
    <div className="flex-1 space-y-6 p-6 md:p-10 bg-stone-50/50 dark:bg-stone-900/50 min-h-screen text-stone-900 dark:text-stone-100">
      ...
      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
        <div className="space-y-1">
          ...
          <p className="text-sm text-stone-500 dark:text-stone-400 max-w-xl">
            Track sales, update fulfillment status, and input shipping detail logs.
          </p>
        </div>
        <Button
          variant="outline"
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-6">
        <Card className="bg-white dark:bg-stone-950 border-stone-200 dark:border-stone-800 shadow-sm hover:shadow-md transition-shadow duration-200 rounded-lg relative overflow-hidden group">
          ...
          <CardContent className="pt-6 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Total Orders</span>
              <p className="text-3xl text-stone-950 dark:text-stone-50 font-extrabold">{items.length}</p>
            </div>
        ...
        <Card className="bg-white dark:bg-stone-950 border-stone-200 dark:border-stone-800 shadow-sm hover:shadow-md transition-shadow duration-200 rounded-lg relative overflow-hidden group">
          ...
          <CardContent className="pt-6 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Pending Processing</span>
              <p className="text-3xl text-stone-950 dark:text-stone-50 font-extrabold">
                {items.filter(o => o.status === "PENDING" || o.status === "PROCESSING").length}
              </p>
            </div>
        ...
        <Card className="bg-white dark:bg-stone-950 border-stone-200 dark:border-stone-800 shadow-sm hover:shadow-md transition-shadow duration-200 rounded-lg relative overflow-hidden group">
          ...
          <CardContent className="pt-6 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Delivered Total</span>
        ...
        <Card className="bg-white dark:bg-stone-950 border-stone-200 dark:border-stone-800 shadow-sm hover:shadow-md transition-shadow duration-200 rounded-lg relative overflow-hidden group">
          ...
          <CardContent className="pt-6 flex items-center justify-between">
            <div className="space-y-1">
              <span className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Gross Revenue</span>
      ...
      <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center bg-white dark:bg-stone-950 border border-stone-200 dark:border-stone-800 p-4 rounded-lg shadow-sm">
        <div className="relative w-full sm:max-w-md">
          <Search size={16} className="absolute left-3 top-1/2 -translate-y-1/2 text-stone-450 dark:text-stone-500" />
          <Input
      ...
      <div className="border border-stone-200 dark:border-stone-800 rounded-lg bg-white dark:bg-stone-950 overflow-hidden shadow-sm">
        {isLoading ? (
          ...
          <div className="h-64 flex justify-center items-center flex-col gap-2 p-6 text-center">
            <p className="text-sm font-medium text-rose-600">Failed to load orders list from server.</p>
              ...
              <Table>
                <TableHeader className="bg-stone-100/50 dark:bg-stone-900/50">
                  <TableRow className="border-b border-stone-200 dark:border-stone-800">
                    ...
                    <TableHead className="font-mono text-stone-500 dark:text-stone-400 uppercase tracking-widest text-[10px] py-4">Payment Status</TableHead>
                    <TableHead className="font-mono text-stone-500 dark:text-stone-400 uppercase tracking-widest text-[10px] py-4">Fulfillment</TableHead>
                    <TableHead className="font-mono text-stone-500 dark:text-stone-400 uppercase tracking-widest text-[10px] py-4 text-right">Amount</TableHead>
                    <TableHead className="font-mono text-stone-500 dark:text-stone-400 uppercase tracking-widest text-[10px] py-4 text-center">Actions</TableHead>
                  </TableRow>
                </TableHeader>
                <TableBody className="divide-y divide-stone-100 dark:divide-stone-900">
                  {filteredItems.map((order) => (
                    <TableRow key={order.id} className="border-b border-stone-200 dark:border-stone-800 hover:bg-stone-50/50 dark:hover:bg-stone-900/30 transition-colors">
                      ...
                      <TableCell className="py-4">
                        <div className="flex flex-col gap-0.5">
                          ...
                          <span className="text-[10px] text-stone-400 dark:text-stone-500 font-light">{order.phoneNumber}</span>
                        </div>
                      </TableCell>
                      <TableCell className="py-4">
      ...
      <Dialog open={isUpdateDialogOpen} onOpenChange={setIsUpdateDialogOpen}>
        <DialogContent className="bg-white dark:bg-stone-950 border border-stone-200 dark:border-stone-800 rounded-lg p-6 max-w-md">
          <DialogHeader>
            <DialogTitle className="text-lg font-bold text-stone-955 dark:text-stone-50">Update Fulfillment Log</DialogTitle>
            <DialogDescription className="text-xs text-stone-400">
              Modify the order status and append logistics information for Order #{selectedOrder?.orderNumber}.
            </DialogDescription>
          </DialogHeader>

          <div className="space-y-4 py-4">
            <div className="space-y-1.5 flex flex-col">
              <label className="text-[10px] font-bold uppercase tracking-wider text-stone-400">Fulfillment Status</label>
              <Select value={updateStatus} onValueChange={setUpdateStatus}>
                <SelectTrigger className="w-full bg-white dark:bg-stone-950 border-stone-200 dark:border-stone-800 text-xs">
          ...
          <DialogFooter className="gap-2 pt-4 border-t border-stone-100 dark:border-stone-850">
            <Button
              variant="outline"
            ...
            >
              {isUpdating ? "Saving Log..." : "Commit Update"}
            </Button>
          </DialogFooter>
        </DialogContent>
      </Dialog>
